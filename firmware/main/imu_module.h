...
private:
    ...
    /**
     * @brief 6 frame calibr for accel
     * @param en
     * @return int
     */
...
public:
    ...
    TickType_t get_timestamp() { return timestamp; };
    void start_gyro_static_calibr() { run_gyro_static_calibr = true; };
    bool get_gyro_static_calibr_status() { return run_gyro_static_calibr; };
    void start_gyro_z_scale_calibr() { run_gyro_scale_calibr = true; };
    bool get_gyro_z_scale_calibr_status() { return run_gyro_scale_calibr; };
    void start_aceel_sphere_calibr() { run_accel_calibr = true; };
    bool get_accel_calibr_status() { return run_accel_calibr; };
    float get_gyro_x() { return gyro[ROLL]; };
    float get_gyro_y() { return gyro[PITCH]; };
    float get_gyro_z() { return gyro[YAW]; };
    float get_accel_x() { return accel[ROLL]; };
    float get_accel_y() { return accel[PITCH]; };
    float get_accel_z() { return accel[YAW]; };
    struct quaternion get_quaternion() { return q; };
    float get_euler_roll() { return (euler_zyx[ROLL] = atan2(2 * (q.w * q.x + q.y * q.z), -2 * (q.x * q.x + q.y * q.y) + 1)); }
    float get_euler_pitch() { return (euler_zyx[PITCH] = asin(2 * (q.w * q.y - q.z * q.x))); }
    float get_euler_yaw() { return (euler_zyx[YAW] = atan2(2 * (q.w * q.z + q.x * q.y), 1 - 2 * (q.y * q.y + q.z * q.z))); }
